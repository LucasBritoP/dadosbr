...
Este documento mapeia o que o DadosBR entrega hoje pela API HTTP e pelo servidor MCP. Ele serve como uma visao de produto e tambem como guia rapido para agentes, notebooks, pipelines e aplicacoes que precisam consumir dados publicos do mercado brasileiro.

Status atual da alpha:

- API HTTP FastAPI com 71 paths publicos de dados, sem contar `/docs`, `/redoc` e `/openapi.json`.
...
- Fontes oficiais ou gratuitas quando disponiveis; enriquecimentos externos ficam identificados como tal.
- Operacoes administrativas, leitura de arquivos locais e download automatico B3 ficam desligados por padrao.

## Quando Usar API ou MCP

| Superficie | Melhor para | Como consumir |
| --- | --- | --- |
| API HTTP | Aplicacoes, dashboards, notebooks, jobs, pipelines e integracoes entre servicos. | `dadosbr-api`, FastAPI, OpenAPI, `curl`, `httpx`, clientes REST. |
...
| --- | --- | --- | --- |
| Fontes e metadados | Catalogo de fontes, licencas, frequencia, notas e limitacoes. | `GET /sources`, `GET /sources/{source_id}` | `list_sources`, `get_source_metadata` |
...
| Tesouro Transparente | Taxas e precos publicos do Tesouro Direto. | `GET /tesouro/rates` | `get_tesouro_rates` |
...
| BNDES | Operacoes de financiamento, renda variavel, debentures, fundos e catalogo CKAN. | `GET /bndes/operations`, `GET /bndes/variable-income`, `GET /bndes/datasets` | `get_bndes_operations`, `get_bndes_variable_income`, `list_bndes_datasets` |
...
| Calendario macro | Regras de frequencia e divulgacao de indicadores macro. | `GET /macro/calendar` | `list_macro_calendar` |
...
| --- | --- |
| B3 COTAHIST automatico | Exige aceite dos termos de Market Data da B3; use `DADOSBR_ALLOW_B3_DOWNLOAD=1` apenas apos aceite. |
...
1. chamar `list_sources` para explicar fontes e licencas;
...
- CVM, Receita, B3, MDIC e BNDES podem oscilar, mudar layout ou ficar indisponiveis.
...
- 73 tools MCP executadas, com `admin_sync_cnpj` bloqueado por padrao como erro esperado.
